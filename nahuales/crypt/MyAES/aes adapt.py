...
def encryptData(key, data, mode=AESModeOfOperation.modeOfOperation["CBC"]):
	...
	#return bytes(list(map(chr, iv)) + list(map(chr, ciph)))
	return bytes(iv + ciph)
...
def decryptData(key, data, mode=AESModeOfOperation.modeOfOperation["CBC"]):
	...
	iv = [x for x in data[:16]]
	#data = list(map(ord, data[16:]))
	data = [x for x in data[16:]]
	moo = AESModeOfOperation()
...
if __name__ == "__main__":
	...
	cypherkey = bytes([143,194,34,208,145,203,230,143,177,246,97,206,145,92,255,84])
	iv = bytes([103,35,148,239,76,213,47,118,255,222,123,176,106,134,98,92])
